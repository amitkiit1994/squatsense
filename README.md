...
Squat rep counter and biomechanics metrics from video or **live webcam**, using MediaPipe Pose (CPU-only, macOS-friendly). Uses **3D world landmarks** when available for view-invariant angles and depth — metrics stay accurate regardless of camera angle.
...
The report can include short, actionable coaching cues generated by an LLM. This is **off by default** and requires two environment variables:

```bash
export AI_COACH_ENABLED=1
export OPENAI_API_KEY=sk-...
```

When enabled, the AI coach summarizes your rep metrics and returns 3-5 bullet-point cues (depth, trunk lean, tempo, etc.). The model defaults to `gpt-4.1`; override with `OPENAI_MODEL`. If the API is unreachable or the key is missing, the report is generated normally without coaching.
...
Tests cover 3D geometry helpers, metric computation, batch rep detection, and the incremental rep detector.
